...
message(paste("Current context:", context))

if (context == "TEST" | context == "QA" | context == "PRODUCTION") {
  message("Processing at Rapporteket")
  kb <- les_oqr_kb(system.file("SmerteReg_klokeboken.05.10.2017.csv", package = "smertereg"))
  d_full <- smerteGetRegDataFromRapporteketDb()
  #d_full <- transformRegData(dat, kb)
} else {
...
linje_tilsyn_per_maaned = function(d, type_tilsyn, ylabtekst){
  ...
  # Are: how can this cast to Posix (as.Date) possibly work? yearmonth is defined a string like "Nov. 2017"
  #d = d %>% dplyr::mutate(yearmonth_date = as.Date(yearmonth), yearmonth = as.character(yearmonth))
  # Suggest something like, but may not provide what is needed:
  d = d %>% dplyr::mutate(yearmonth_date = mce_registered_date, yearmonth = as.character(yearmonth))
...
# Finne summen av hver tilsynstype
total_tilsyn_avdeling =  dplyr::summarise_all(sum_tilsyn_avdeling[,-1], funs(sum))
