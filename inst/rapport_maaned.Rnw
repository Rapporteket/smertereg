...
if (context == "TEST" | context == "QA" | context == "PRODUCTION") {
  ...
  d_full <- smerteGetRegDataFromRapporteketDb(reshID)
...
NB! Denne informasjonen MÅ være med i rapporten (justeres etter ønske, navn
er sikkert bedre å bruke enn tall/kode): data er hentet fra innleggelse ved
følgende avdelinger:
\Sexpr{paste(unique(d_full$emp11_department), collapse = ', ')}.
