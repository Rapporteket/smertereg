...
if (context == "TEST" | context == "QA" | context == "PRODUCTION") {
  ...
  d_full <- getRegDataTilsyn(reshID)
...
d_full = d_full %>% dplyr::mutate(avdeling_navn = case_when(emp11_department == 4 ~ "Medisinsk p1 Nord",
                                                     ...
                                                     #Her slutter Helse Bergen
                                                           ...
                                                           FALSE ~ "Ikke angitt"),
                                 tilsett = dplyr::case_when(mce_supervision == 1 ~ "Ja", 
...
linje_tilsyn_per_maaned = function(d, type_tilsyn, ylabtekst){
  ...
  graf0 = ggplot(d, aes(x = yearmonth_date, y  = snitt, ymin = min, ymax = maks)) +
    ...
    #tema +
    #fjern_x +
    geom_linerange(size=.5, colour="darkorchid1") +
